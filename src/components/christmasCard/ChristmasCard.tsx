...
function ChristmasCard({ isAnimationInView }: AnimationProps) {
  ...
  const foldedCardHeight = envRef.current
    ? envRef.current.getBoundingClientRect().height * 0.9
    : 0;

  const envFlapRotateX = useTransform(
    scrollYProgress,
    [0, flapOpen, moveEnvLeftStart, moveEnvLeftEnd - 0.09],
    [0, 180, 180, 0]
  );
  const envRotate = useTransform(
    scrollYProgress,
    [flapOpen, rotateRight, rotateLeft],
    [0, 20, 0]
  );
  const envX = useTransform(
    scrollYProgress,
    [moveEnvLeftStart, moveEnvLeftEnd],
    [0, -(windowSize.width / 2 + envWidth / 2)]
  );
  const cardLeftAnimate = useAnimation();
  const cardLeftRotateX = useTransform(
    scrollYProgress,
    [0, rotateRight, rotateLeft],
    [0, 0, 40]
  );
  const CardY = useTransform(
    scrollYProgress,
    [flapOpen, rotateRight, rotateLeft],
    [0, -(foldedCardHeight + 50), 0]
  );
    ...
    <S.Bg ref={bgRef}>
      <S.StackElements
        ...
        style={{
          rotate: envRotate,
          top:
            bgRef.current && envRef.current
              ? //Center horizontally
                bgRef.current.getBoundingClientRect().height / 2 -
                envRef.current.getBoundingClientRect().height / 2
              : 0,
        }}
      >
        <S.EnvelopeBack
        ...
        >
          <S.StackCards>
            ...
            <S.CardLeftFront
              ...
              style={{
                rotateX: cardLeftRotateX,
                willChange: isAnimationInView
                  ? 'transform, box-shadow'
                  : undefined,
              }}
            />
            <S.CardRightFront src={christmasCardFront} />
          </S.StackCards>
        </S.CardCont>
        <S.EnvelopeFlap
          ref={envFlapRef}
